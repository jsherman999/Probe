...
class GameManager {
    ...
    async getBotStats() {
        ...
        for (const filename of jsonFiles) {
            ...
            const botPlayers = data.players?.filter((p) => p.isBot) || [];
            for (const bot of botPlayers) {
                // Find bot's result (placement and final score)
                const result = data.results?.find((r) => r.playerId === bot.botId || r.displayName === bot.displayName);
                // Count correct/incorrect guesses from turns
                const botTurns = data.turns?.filter((t) => t.playerId === bot.botId) || [];
                const correctGuesses = botTurns.filter((t) => t.isCorrect).length;
                const incorrectGuesses = botTurns.filter((t) => !t.isCorrect).length;
                // Calculate accuracy
        ...
        // Sort by date descending
        return botStats.sort((a, b) => new Date(b.completedAt || 0).getTime() - new Date(a.completedAt || 0).getTime());
    }
