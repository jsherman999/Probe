...
const WordHistory_1 = require("../WordHistory");
...
The word should be valid but challenging to guess.`,
        ...
        const recentWords = Array.from(WordHistory_1.wordHistory.getUsedWords()).slice(-50);
...
No explanation, no punctuation, no quotes - just the single word on one line.`;
        ...
        while (attempts < maxAttempts) {
            try {
                const response = await this.llm.generate(config.modelName, prompt, { ...config.ollamaOptions, temperature: 0.9 + (attempts * 0.1) }, // Increase temperature on retries
                systemPrompt);
                ...
                if (word && word.length >= 4 && word.length <= 12) {
                    ...
                    if (WordHistory_1.wordHistory.isWordUsed(word)) {
                    ...
                    if (isValid) {
                        ...
                        WordHistory_1.wordHistory.addWord(word, config.id, ctx.roomCode);
     ...
     */
    selectFallbackWord(ctx, config) {
        const allWords = FALLBACK_WORDS[config.difficulty] || FALLBACK_WORDS.medium;
        // Filter out already used words
        const availableWords = WordHistory_1.wordHistory.filterUnused(allWords);
        if (availableWords.length === 0) {
            ...
            const anyAvailable = WordHistory_1.wordHistory.filterUnused(allFallbacks);
