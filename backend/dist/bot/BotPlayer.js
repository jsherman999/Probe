...
class BotPlayer {
     ...
     */
    calculateThinkingDelay(difficulty) {
        switch (difficulty) {
            case 'easy':
     ...
     */
    async simulateThinking(overrideMs) {
     ...
     */
    async selectBlankPosition(positions, ctx) {
     ...
     */
    async selectDuplicatePosition(positions, letter, ctx) {
        console.log(`[${this.displayName}] Selecting position for duplicate "${letter}" from: ${positions.join(', ')}`);
        await this.simulateThinking(800);
        try {
            const position = await this.strategy.selectDuplicatePosition(positions, letter, ctx, this.config);
