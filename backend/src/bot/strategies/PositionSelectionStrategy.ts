...
export class PositionSelectionStrategy {
   ...
   */
  private selectByScoring(positions: number[]): number {
   ...
   */
  private async selectStrategicDuplicatePosition(
    positions: number[],
    letter: string,
    ctx: GameContext,
    config: BotConfig
    ...
    const wordDisplay = myWord
      ...
      .map((char, idx) => {
        ...
        if (positions.includes(idx)) return `[${char}]`; // Mark positions with the duplicate letter
        return '•';
      })
      .join('');
